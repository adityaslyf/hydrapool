...
interface UseFriendsReturn {
  ...
  getFriendStatus: (friendId: string) => Promise<'none' | 'pending_sent' | 'pending_received' | 'friends'>;
...
export function useFriends(): UseFriendsReturn {
  ...
  const sendFriendRequest = useCallback(async (friendId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return { success: false, error: errorData.error || 'Failed to send friend request' };
      }
  ...
  const checkExistingRelation = useCallback(async (friendId: string): Promise<FriendRequest | null> => {
    ...
    try {
      ...
      return relations.find(
        (relation) => 
          relation.otherUser.id === friendId
      ) || null;
    } catch (error) {
  ...
  const getFriendStatus = useCallback(async (friendId: string): Promise<'none' | 'pending_sent' | 'pending_received' | 'friends'> => {
